          ...
          FilesAdded: (up, files) => {
            plupload.each(files, (file) => {
              ...
              document.getElementById('ossfile').innerHTML += `<div id="${file.id}">${file.name} (${plupload.formatSize(
                file.size,
              )})<b></b><div class="progress"><div class="progress-bar" style="width: 0"></div></div></div>`;
              that.message = '';
            });
          },
            ...
            if (info.status === 200) {
              document
                ...
                .getElementsByTagName(
                  'b',
                )[0].innerHTML = `upload to oss success, object name:${this.getUploadedObjectName(file.name)}`;
            } else {
